...
func DataPg() {
    ...
    for i, role := range roles {
        ...
        err := global.Pg.Where("id = ?", id).First(&oldRole).Error
    ...
    for i, user := range users {
        ...
        err := global.Pg.Where("id = ?", id).First(&oldUser).Error
        if errors.Is(err, gorm.ErrRecordNotFound) {
            user.Id = id
    ...
    for i, api := range apis {
        ...
        err := global.Pg.Where("id = ?", id).First(&oldApi).Error
        if errors.Is(err, gorm.ErrRecordNotFound) {
            api.Id = id
            api.Creator = creator
